<template>
    ...
    >
        ...
        <v-card class="elevation-2 --anim-2">
            ...
            <v-img :src="imgC(data.img)" :lazy-src="imgC(data.img, false, false, true)" height="40%"
                   :ref="'tl-img-' + index">
                <v-icon size="150px" color="primary" class="ma-4">{{data.icon}}</v-icon>
            </v-img>
            <div>
                <v-card-title class="headline px-5 pt-5">{{data.subTitle}}</v-card-title>
                <v-card-text class="px-5 pb-5">
                    {{data.subSubTitle}}
                </v-card-text>
            </div>
        </v-card>
        <div slot="opposite" class="--anim-1" v-if="!$vuetify.breakpoint.smAndDown">
            <h1 class="opposite_tl --anim-3">{{data.title}}</h1>
        </div>
    </v-timeline-item>
</template>
            ...
            scrollTops() {
                ...
                if (pos < window.innerHeight / 1.5 && !$e.scrollAnimated) {
                    ...
                    tL = new TimelineMax()
                        ...
                        .set($e.querySelector('.--anim-1'), {y: 50, autoAlpha:0})
